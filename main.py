...
# problems to resolve without asking a robot:
     # being spawned in a random location without preparation
     # resizing having SO many glitches
     # fix the broken enemies.... <//3
     # WHY WHY WHY does the screen flicker so often??????? is it a blit issue???? it hurts my eyes

# MAYBE fix these too:
    # fix the partner/player/bot thing (a class that gets inherited from >>>>>>> whatever these separate classes are...)
    # get rid of kooky comments and old dead code
    # improve level generation thing (potentially procedural level generation? i'm too dumb for that...)
...
class Game:
    def __init__(self):
        ...
        self.running = True
        self.state = "menu"    # from the states pf menu, credits, playing, lev2
        self.title_font = pygame.font.SysFont("Albertus Nova Black", 100)
        self.menu_font = pygame.font.SysFont("Albertus Nova", 60)    # i wonder how this'll work on other computers that don't have crazy fonts
...
# THE =(unfinished)credits i placed smack in the middle fo my code!!!! yeah1!!!!!
    ...
    # actual game loop thing
    def play(self):
        spritesheet = Spritesheet('nnnn.png')
        player = Player()
        partner = Partner()
        tile_map = TileMap("branchlev.csv", spritesheet)#('branchlev.csv', spritesheet)
...
########################### broken broken broken
            ...
            # eenemies that don't really do much except swoop around (BROKEN< DO NOT TOUCH UNLESS YOU KNOW WHAT YOUR"E DOING)
            # for e in enemies:
            ...
            # there;s an actual goalpost in this one!!!!!!!
            for g in goals:
                if player.rect.colliderect(g.rect):
                    print("Level complete!")
                    self.state = "lev2" #"done with intro" 
                    # import lev2
...
####################### commenting enemies out because the mechanic is broken... and stolen from a tutorial... ow
            # for e in enemies:
            #     if player.rect.colliderect(e.rect):
            #         player.lives -= 1
            #         if player.lives <= 0:
...
#runs  (made it run from a different file game py)
